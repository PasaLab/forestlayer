...
def parameter_server(num_workers=2,
	...
	with tf.device("/job:{}/task:{}".format(job_name, task_index)):
		...
		cont_merge = tf.Variable([True for _ in range(num_mergers)], name='continue')
	...
	while True:
		...
		for i in range(0, num_workers, num_split):
			# sess.run(prob_train_xs[i].assign_add(prob_train_xs[i + 1]))
			# sess.run(prob_test_xs[i].assign_add(prob_test_xs[i + 1]))
		...
		sess.run(merged.assign([False for _ in range(num_mergers)]))
		...
		sess.run(cont_merge.assign([True for _ in range(num_mergers)]))
...
def merger(num_workers=2,
	...
	num_layer = 0
	while True:
		print("[Merger {}] Start merge Layer {} ......".format(task_index, num_layer))
		while not sess.run(ready).all():
			sleep(1.0)
		print("All Ready!!! Begin to Merge ...")

		# now merging, close merging tag ============================================
		indices = tf.constant([[task_index], ])
		updates = tf.constant([False])
		set_cont_merge = cont_merge.scatter_nd_update(indices, updates)
		sess.run(set_cont_merge)
		# =============================================================================
		...
		print("[Merger {}] wait the indicator of the next merging ...".format(task_index))
		is_shutdown = False
		while not sess.run(cont_merge)[task_index]:  # while not continue
			time.sleep(1.0)
			print("cont indicator: ", sess.run(cont_merge)[task_index])
			if sess.run(shut_down):
				is_shutdown = True
				break
		if is_shutdown:
			print("Merger %d shutting down..." % task_index)
			break
		num_layer += 1
