...
class SmallNORBExample:
    def __init__(self):
        """
        Code from https://github.com/ndrplz/small_norb.
        6 lighting conditions, 9 elevations (30 to 70 degrees every 5 degrees),
         and 18 azimuths (0 to 340 every 20 degrees). 6 + 1 + 1 = 8 dims
        """
        self.image_lt = None
        self.image_rt = None
        self.category = None
...
class SmallNORBDataset(object):
    ...
    def export_to_jpg(self, export_dir):
        ...
        """
        if self.initialized:
            print('Exporting images to {}...'.format(export_dir))
    ...
    def matrix_type_from_magic(magic_number):
        ...
        magic_str = binascii.b2a_hex(bytearray(reversed(magic_number))).upper()
...
def load_data(data_root):
    """
    Load data from data_root directory.

    :param data_root: dataset root dir.
    :return: (x_train, y_train, x_train_plus), (x_test, y_test, y_test_plus),
              with shape ((24300, 2, 96, 96), (24300,), (24300, 8))
    """
    dataset = SmallNORBDataset(dataset_root=data_root)
    x_train_norb = dataset.data['train']
    x_test_norb = dataset.data['test']
    x_train, y_train, x_train_plus = transform_phase(x_train_norb)
    x_test, y_test, y_test_plus = transform_phase(x_test_norb)
    return (x_train, y_train, x_train_plus), (x_test, y_test, y_test_plus)


def transform_phase(x_norb):
    """
    ...
    :return:
    """
    x = np.zeros((24300, 2, 96, 96), dtype=np.uint8)
    y = np.zeros((24300,), dtype=np.uint8)
    x_plus = np.zeros((24300, 8), dtype=np.int16)
    for i in range(24300):
        x[i, 0, :, :] = x_norb[i].image_lt
        x[i, 1, :, :] = x_norb[i].image_rt
        y[i] = x_norb[i].category
        x_plus[i, x_norb[i].lighting] = 1
        x_plus[i, 6] = x_norb[i].elevation
        x_plus[i, 7] = x_norb[i].azimuth
    return x, y, x_plus
